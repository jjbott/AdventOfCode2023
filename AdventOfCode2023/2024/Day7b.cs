...
namespace AdventOfCode2023._2024
{
    internal class Day7b
    {
        public List<long> Calc(List<long> values, long max)
        {
            ...
            if (values.Count == 2)
            {
                return new List<long>() { values[0] + values[1], values[0] * values[1], long.Parse(values[0].ToString() + values[1].ToString()) };
            }
            ...
            var newValues3 = new List<long>() { long.Parse(values[0].ToString() + values[1].ToString()) };
            newValues3.AddRange(values.Skip(2));

            var answers = new List<long>();
            answers.AddRange(Calc(newValues1, max));
            answers.AddRange(Calc(newValues2, max));
            answers.AddRange(Calc(newValues3, max));
